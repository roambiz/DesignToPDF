...
<p>这是单个程序，适用于 64 位 Windows 7 SP1 至 Windows 11。32 位系统不支持。先保存到桌面再双击打开，不要在微信聊天里直接点开。若直接从聊天拖入文件，输出会放到桌面「导出结果」，避免微信清理后找不到。也可以拖入 zip，程序会展开里面的设计文件；rar / 7z 请先解压。从压缩软件预览窗口或网盘未下完的位置拖入时，会尽量复制到本地再处理。部分电脑第一次打开会比较慢，属正常现象；窗口出来之后即可拖入文件。</p>
